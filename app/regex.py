...
RE_SUBWORD_BOUNDARY_FWD = re.compile(
    "(?:[_-]?[A-Z][a-z-]+)|(?:[_-]?[A-Z]+(?![a-z]))|(?:[_-]?[a-z]+)|(?:\W+)"
)

RE_SUBWORD_BOUNDARY_RVR = re.compile(
    "(?:[A-Z][a-z-]+[_-]?)|(?:[A-Z]+(?![a-z])[_-]?)|(?:[a-z]+[_-]?)|(?:\W+)"
)

RE_WORD_BOUNDARY = re.compile("(?:\w+)|(?:\W+)")
...
RE_NUMBERS = re.compile(RE_NUMBERS)
...
RE_ENGLISH_CONTRACTION = r"(\"(\\\"|[^\"])*?\")|(?<![adegIlnotuwy])('(\\\'|[^'])*?')"
